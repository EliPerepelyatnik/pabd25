...
def parse_cian():
    ...
    n_rooms = 1
    csv_path = f'data/raw/{n_rooms}_flats.csv'
...
def preprocess_data():
    ...
    new_df = df[df['total_meters'] < 300]

    os.makedirs('../data/processed', exist_ok=True)
    df.to_csv('../data/processed/data_cleaned.csv', encoding='utf-8')
...
def main():
    parser = argparse.ArgumentParser(description="Предсказание цен на жилье")
    ...
    train_test_model(model_name=args.model)
